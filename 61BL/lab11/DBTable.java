...
public class DBTable<T> {
    ...
    public <R extends Comparable<R>> List<T> getOrderedBy(Function<T, R> getter) {
        return entries.stream().sorted(Comparator.comparing(getter)).collect(Collectors.toList());
    }
    ...
    public <R> List<T> getWhitelisted(Function<T, R> getter, Collection<R> whitelist) {
        return entries.stream().filter(s -> whitelist.contains(getter.apply(s))).collect(Collectors.toList());
    }
    ...
    public <R> DBTable<R> getSubtableOf(Function<T, R> getter) {
        return new DBTable<>(entries.stream().map(getter).collect(Collectors.toList()));
    }
...
//        List<User> whiteListed = t.getWhitelisted(User::getName, whiteListedNames);
//        System.out.println(whiteListed);

//        DBTable<String> names = t.getSubtableOf(User::getName);
//        System.out.println(names.getEntries());
    }
}
