...
//        System.out.println(family.longestName());
    ...
    public static class Amoeba {
        ...
        public String longestNameHelper() {
            ...
            for (Amoeba a : children) {
                String longestNameSeen = a.longestNameHelper();
                if (longestName.length() < longestNameSeen.length()) {
                    longestName = longestNameSeen;
                }
            }
            return longestName;
        }
        ...
        public void printHelper(String indentation) {
            System.out.println(indentation + name);
            for (Amoeba a : children) {
                a.printHelper(indentation + "    ");
            }
        }
    ...
    public class AmoebaDFSIterator implements Iterator<Amoeba> {
        ...
        public Amoeba next() {
            Amoeba item = fringe.pop();
            if (item.children != null) {
                for (int i = item.children.size() - 1; i >= 0; i--) {
                    fringe.push(item.children.get(i));
                }
            }
            return item;
        }
    ...
    public class AmoebaBFSIterator implements Iterator<Amoeba> {
        ...
        /* Returns the next element. */
        public Amoeba next() {
            Amoeba item = fringe.remove();
            if (item.children != null) {
                fringe.addAll(item.children);
            }
            return item;
        }
