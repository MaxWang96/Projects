...
public class Graph {
    ...
    public Graph prims(int start) {
        Graph MST = new Graph();
        MST.addVertex(start);
        Set<Edge> nei = edges.get(start);
        PriorityQueue<Edge> fringe = new PriorityQueue<>(nei);
        Edge toAdd;
        Set<Integer> visited = new HashSet<>();
        visited.add(start);
        while (visited.size() != edges.size() && !fringe.isEmpty()) {
            toAdd = fringe.poll();
            if (!visited.contains(toAdd.getDest())) {
                MST.addEdge(toAdd);
                visited.add(toAdd.getDest());
                nei = edges.get(toAdd.getDest());
                for (Edge e : nei) {
    ...
    public Graph kruskals() {
        Graph MST = new Graph();
        edges.keySet().forEach(MST::addVertex);
        PriorityQueue<Edge> fringe = new PriorityQueue<>(allEdges);
        UnionFind union = new UnionFind(edges.size());
        Edge toAdd;
        while (union.sizeOf(0) != edges.size()) {
            toAdd = fringe.poll();
            if (!union.connected(toAdd.getDest(), toAdd.getSource())) {
                MST.addEdge(toAdd);
                union.union(toAdd.getDest(), toAdd.getSource());
            }
        }
        return MST;
    }
