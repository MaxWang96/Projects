...
public class DLList<T extends Comparable<T>> {
    ...
    private void insertionSortHelper(T item) {
        addLast(item);
        for (Node tmp = sentinel.prev; tmp.prev != sentinel && tmp.item.compareTo(tmp.prev.item) < 0; tmp = tmp.prev) {
            swap(tmp.prev, tmp);
            tmp = tmp.next;
        }
    }

    /* n1 should be the previous item of n2. The sequence of input can't be changed */
    ...
    public DLList<T> mergeSort() {
        ...
        Node toAdd = sentinel.next;
        for (int i = 0; i < size / 2; i++) {
            oneHalf.addLast(toAdd.item);
            toAdd = toAdd.next;
        }
        for (int i = 0; i < size - (size / 2); i++) {
            otherHalf.addLast(toAdd.item);
            toAdd = toAdd.next;
        }
    ...
    public DLList<T> quicksort() {
        ...
        Node tmp = sentinel.next.next;
        equalElements.addLast(pivot);
        for (int i = 1; i < size; i++) {
            if (tmp.item.compareTo(pivot) < 0) {
                smallElements.addLast(tmp.item);
            } else if (tmp.item.compareTo(pivot) > 0) {
                largeElements.addLast(tmp.item);
            } else {
                equalElements.addLast(tmp.item);
            }
            tmp = tmp.next;
        }
