...
public class AugmentedStreetMapGraph extends StreetMapGraph {
    private final Map<Point, Node> pointMap = new HashMap<>();
    ...
    public AugmentedStreetMapGraph(String dbPath) {
        ...
        for (Node n : nodes) {
            if (neighbors(n.id()).size() != 0) {
                pointMap.put(new Point(projectToX(n.lon(), n.lat()), projectToY(n.lon(), n.lat())), n);
            }
        }
        List<Node> nodesAC = this.getAllNodes();
        for (Node m : nodesAC) {
            if (m.name() != null) {
                trie.add(m);
            }
        }
        tree = new KDTree(new LinkedList<>(pointMap.keySet()));
    }
    ...
    public long closest(double lon, double lat) {
        ...
        Point nearest = tree.nearest(x, y);
        return pointMap.get(nearest).id();
    }
    ...
    public List<String> getLocationsByPrefix(String prefix) {
        return new LinkedList<>(trie.keysWithPrefix(prefix));
    }
    ...
    public List<Map<String, Object>> getLocations(String locationName) {
        return trie.getLocations(locationName);
    }
